...
export const ConversationProvider = ({ children }) => {
    ...
    const fetchConversation = async (id) => {
        // console.log(id);
        try {
            const response = await fetch(`http://localhost:3000/api/users/conversation/${id}`);
    ...
    const continue_conversation = async (id, conversationId, query) => {
        ...
        try {
            ...
            const response = await axios.post(url, payload,  {
                headers: {
                    Authorization: `Bearer ${auth.token}`, // Add token to Authorization header
                },
            })
    ...
    return (
        <ConversationContext.Provider value={{ conversationData, fetchConversation, conversationId, continue_conversation }}>
            {children}
